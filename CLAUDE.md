...
- **Normal**: frame navigation (←/→, and Shift+←/→ to jump ±10 frames clamped — `FRAMES_PER_JUMP`), `f` opens the frame sub-menu, g presentation settings (frame size), Ctrl-s save, Shift+S **save as** (centred filename popup `Mode::SaveAs` → `state::save_as`, adopting the new path; a plain capital `S` like `F`=fullscreen, so it works on every terminal — `Ctrl-Shift-s` was undetectable without keyboard-enhancement; still checked before plain save so a custom `Ctrl-Shift-s` binding wins over `Ctrl-s` on enhanced terminals), q quit, Shift+F fullscreen, Shift+C **converge** (see below)
...
- **Converge** (`Shift+C` in Normal, configurable as `converge`): animate a set
  of objects so they all meet on **one shared point**, each starting from
  *wherever it happens to be* at the span start. Reuses the multi-select toggle
  flow (`MultiSelectPurpose::Converge`): `Space` toggles members (a selected
  `Group` pulls in its members via `expand_selection`), `Enter` opens
