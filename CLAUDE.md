...
Normal ──a──→ AddObject ──Enter──→ Normal (object added)
       ──s──→ MultiSelect{Select} ──Enter(1 obj)──→ SelectedObject ──e──→ EditProperties ──a──→ AnimateProperty
                                  ──Enter(2+ obj)─→ SelectAction (Copy / Converge)
```
...
- **SelectAction**: the action sub-menu shown after selecting 2+ objects (`SELECT_ACTIONS`, ↑/↓ + Enter). Currently **Copy** (`copy_to_clipboard`) and **Converge** (`expand_selection` → `enter_converge`). This is where copy & converge moved to from their old top-level `c`/`Shift+C` keys.
...
totals 223 tests (110 integration
+ 113 inline); `TESTS.md` is the authoritative per-test list.
