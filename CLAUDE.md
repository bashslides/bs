...
- **Normal**: frame navigation (←/→, and Shift+←/→ to jump ±10 frames clamped — `FRAMES_PER_JUMP`), `f` opens the frame sub-menu, g presentation settings (frame size), Ctrl-s save, Shift+S **save as** (centred filename popup `Mode::SaveAs` → `state::save_as`, adopting the new path; a plain capital `S` like `F`=fullscreen, so it works on every terminal — `Ctrl-Shift-s` was undetectable without keyboard-enhancement; still checked before plain save so a custom `Ctrl-Shift-s` binding wins over `Ctrl-s` on enhanced terminals), q quit, Shift+F fullscreen
...
totals 198 tests (100 integration
+ 98 inline); `TESTS.md` is the authoritative per-test list.
